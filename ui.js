...
function doPredict(predict) {
  ...
  var normalisedScore = result.score.toFixed(2);
  var sentiment = "Neutral";
  if (normalisedScore > 0.65) {
    sentiment = "Positive";
  } else if (normalisedScore == 0.65) {
    sentiment = "Neutral";
  } else {
    sentiment = "Negative";
  }
  var rating = normalisedScore * 5;
  status('Review sentiment: ' + sentiment + ', Rating (on the scale of 5): ' + rating.toFixed(0));
}
