...
function loadFeatures(filePath, numWords, maxLen, multihot = false) {
  ...
  while (index < numBytes) {
    ...
    //console.log("Adding logger for value", value);
    if (value === 1) {
    ...
    //console.log("Adding logger for Sequence", sequences);
    // console.log("Adding logger for Seq", seq);
    index += 4;
  }
  if (seq.length > 0) {
    sequences.push(seq);
...
export async function loadData(numWords, len, multihot = false) {
  ...
  console.log("Logging the xtrain ", xTrain);
